...
public void OnPluginStart()
{
	...
	cv_smPrompt = CreateConVar("server_sm_prompt", "1", "SM提示仅限管理可见 禁用:0 启用:1", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cv_AFKtoSpec = CreateConVar("server_afk_to_spec", "0", "玩家闲置多少秒移到旁观 禁用:0", FCVAR_NOTIFY, true, 0.0);
...
void Event_playerteam(Event event, const char[] name, bool dontBroadcast) 
{
	...
	if (GetConVarBool(cv_AFKtoSpec))
	{
		...
		{
			if (newteam == 1)
				CreateTimer(GetConVarFloat(cv_AFKtoSpec), timer_AFK, client, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
}
